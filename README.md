...
The way PurgeCSS works is simple - you give it a list of classes that are actually used (well, normally it extract this list itself, from a static html, or by extracting "words" from any text file, like a `.js` app) and then cleans up the css, removing everything it believes is unused.
